...
public class GyAexpedienteDao {

	public List<Object> mostarDatos() {
		List<Object> pacienteD = new ArrayList();
		EntityManager em;
		...
		try {
			...
			pacienteD = em.createQuery("SELECT pa.codigo, pa.nombre_P, pa.apellido, pa.sexo, pa.fecha_De_Nacimiento,"
							...
							+ "INNER JOIN ReservaCita AS reserCita ON reserCita.codigo_Cita = pa.reservaCita.codigo_Cita").getResultList();
	...
	public void agregarD(Paciente pc) {
		EntityManager em;
		EntityManagerFactory emf;
		emf = Persistence.createEntityManagerFactory("Apolo");
		em = emf.createEntityManager();
